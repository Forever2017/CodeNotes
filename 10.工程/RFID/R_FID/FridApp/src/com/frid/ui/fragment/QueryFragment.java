...
public class QueryFragment extends FRFragment implements OnClickListener{
	...
	public void init() {
		...
		QueryVerify = (Button) findViewById(R.id.QueryVerify);
		QueryUpload = (Button) findViewById(R.id.QueryUpload);
		QueryNumber = (Button) findViewById(R.id.QueryNumber);
		QueryGoodTitle = (TextView) findViewById(R.id.QueryGoodTitle);
		QueryGoodCode = (TextView) findViewById(R.id.QueryGoodCode);

		QueryScan.setOnClickListener(this);
		QueryVerify.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		case R.id.QueryCheckScan://扫描
			if(QueryScan.getText().toString().equals(getResources().getString(R.string.Scan))){
				...
				QueryVerify.setEnabled(false);
				QueryVerify.setBackground(getResources().getDrawable(R.drawable.button_circle_gray_style));
				//真实扫描
				device.biBi(true);
				scanFrid();
	...
	/**更新扫描货品的状态*/
	private void updateEPC(boolean b) {
		if(b){
			QueryGoodTitle.setText(currentEPC.getName());
			QueryGoodCode.setText(currentEPC.getNumber());
		}else{
			QueryGoodTitle.setText("");
			QueryGoodCode.setText("");
			currentEPC = null;
		}
	}
