...
public class ActivityRoomList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.recycle_list_general);
    ...
    public void init() {
        ((TextView) findViewById(R.id.ListTitle)).setText(R.string.PvtRoom);
        recyclerView = findViewById(R.id.RecyclerList);

        list = new ArrayList<>();
        adapter = new AdapterRoomList(list, this);
        recyclerView.setAdapter(adapter);        //和gridview一样直接setAdapter
        recyclerView.setLayoutManager(new GridLayoutManager(this, 2)); //这里是设置为网格布局，后边3的参数是列数
        //RecycleView 增加边距
